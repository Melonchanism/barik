...
struct ExperimentalConfig: Decodable {
	...
	init(from decoder: Decoder) throws {
		...
		foreground =
			try container.decodeIfPresent(ForegroundConfig.self, forKey: .foreground)
			?? ForegroundConfig()
		background =
			try container.decodeIfPresent(BackgroundConfig.self, forKey: .background)
			?? BackgroundConfig()
	}
}
...
struct ForegroundConfig: Decodable {
	...
	init(from decoder: Decoder) throws {
		let container = try decoder.container(keyedBy: CodingKeys.self)
		height =
			try container.decodeIfPresent(BackgroundForegroundHeight.self, forKey: .height)
			?? .barikDefault
		horizontalPadding =
			try container.decodeIfPresent(CGFloat.self, forKey: .horizontalPadding)
			?? Constants.menuBarHorizontalPadding
		widgetsBackground =
			try container.decodeIfPresent(WidgetBackgroundConfig.self, forKey: .widgetsBackground)
...
struct BackgroundConfig: Decodable {
	...
	init(from decoder: Decoder) throws {
		...
		if (materialIndex == 7) { type = .black }
		else if (materialIndex == 8) { type = .vignette }
		else { type = .blur }
	}
